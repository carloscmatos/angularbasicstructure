...
.constant('API_URL','http://multifarmas-api-dev-env.us-east-1.elasticbeanstalk.com')
.constant('API_TOKEN', '3c2a7592f070f80d67daeff2560dcf6fc9cf36bd')

//.constant('API_TOKEN', '82c7efa1f9c6216770c68f415ec13adc630172bc')
//.constant('API_URL','http://192.168.99.100:8000')

.config(function(envServiceProvider) {
	// set the domains and variables for each environment 
	envServiceProvider.config({
		domains: {
			development: ['localhost', '192.168.99.100'],
			staging: ['multifarmas-site-angularjs-dev-env.us-east-1.elasticbeanstalk.com'],
			production: ['beta.multifarmas.com.br', 'multifarmas.com.br']
		},
		vars: {
			development: {
				apiUrl: 'http://multifarmas-api-dev-env.us-east-1.elasticbeanstalk.com',
				apiToken: '3c2a7592f070f80d67daeff2560dcf6fc9cf36bd'
			},
			staging: {
				apiUrl: 'http://multifarmas-api-dev.us-east-1.elasticbeanstalk.com',
				apiToken: '3c2a7592f070f80d67daeff2560dcf6fc9cf36bd'
			},
			production: {
				apiUrl: 'http://multifarmas-api-dev.us-east-1.elasticbeanstalk.com',
...
.config(['$stateProvider', function ($stateProvider) {
	...
	$stateProvider
		...
		.state('product-list-index', { // this is a name for our route
			url: '/lista-de-remedios', // the actual url path of the route
	  		templateUrl: 'app/product/product-list-by-first-letter-index.html', // the template that will load
			controller: 'ProductListByFirstLetterCtrl',
		})
		.state('product-list-by-first-letter', { // this is a name for our route
			url: '/lista-de-remedios/letra-:letter', // the actual url path of the route
	  		templateUrl: 'app/product/product-list-by-first-letter-results.html', // the template that will load
			controller: 'ProductListByFirstLetterCtrl',
		})
		
		// Estas sao as duas novas paginas de listagem de farmas e detalhes
		.state('companies-listing', { // this is a name for our route
			url: '/lista-de-farmacias', // the actual url path of the route
	  		templateUrl: 'app/company/company-listing-index.html', // the template that will load
			controller: 'CompanyListingCtrl',
		})	
		.state('companies-details', { // this is a name for our route
			url: '/lista-de-farmacias/:slug?:q=', // the actual url path of the route
	  		templateUrl: 'app/company/company-detail.html', // the template that will load
			controller: 'CompanyDetailsCtrl',
		})	
		// FIM das rotas das duas paginas de listagem e detalhe de farmas

		.state('term-of-usage', { // this is a name for our route
		  	url: '/termos-de-uso', // encontrada actual url path of the route
		  	templateUrl: 'app/statics/term-of-usage.html', // the template that will load
		  	controller:  'TermsOfUseCtrl',
		})
		.state('privacy-policy', { // this is a name for our route
		  	url: '/politica-de-privacidade', // encontrada actual url path of the route
		  	templateUrl: 'app/statics/privacy-policy.html', // the template that will load
		  	controller:  'PrivacyCtrl',
		}) 
		.state('publi-policy', { // this is a name for our route
		  	url: '/politica-de-publicidade', // encontrada actual url path of the route
		  	templateUrl: 'app/statics/publi-policy.html', // the template that will load
		  	controller:  'PubliCtrl',
		}) 
		.state('who-we-are', { // this is a name for our route
		  	url: '/quem-somos', // encontrada actual url path of the route
		  	templateUrl: 'app/statics/who-we-are.html', // the template that will load
		  	controller:  'WhoWeAreCtrl',
		}) 
		// .state('our-partners', { // this is a name for our route
		//   	url: '/nossos-parceiros', // encontrada actual url path of the route
		//   	templateUrl: 'app/statics/our-partners.html', // the template that will load
		//   	controller:  'OurPartnersCtrl',
		// }) 
		// .state('contact-us', { // this is a name for our route
		//   	url: '/entre-em-contato', // encontrada actual url path of the route
		//   	templateUrl: 'app/statics/contact-us.html', // the template that will load
		//   	controller:  'ContactUsCtrl',
		// }) 
		// .state('media-we-have-been', { // this is a name for our route
		//   	url: '/na-midia', // encontrada actual url path of the route
		//   	templateUrl: 'app/statics/media-we-have-been.html', // the template that will load
		//   	controller:  'MidiaWeHaveBeenCtrl',
		// }) 
		.state('product-detail', { // this is a name for our route
			url: '/:slug', // the actual url path of the route
	  		templateUrl: 'app/product/product-detail.html', // the template that will load
			controller: 'ProductCtrl',
		})
		.state('search-with-product-shortname', { // this is a name for our route
			url: '/s/:q', // the actual url path of the route
	  		templateUrl: 'app/search/search.html', // the template that will load
			controller: 'SearchCtrl',
		})		
		.state('search', { // this is a name for our route
			url: '/busca/:q', // the actual url path of the route
	  		templateUrl: 'app/search/search.html', // the template that will load
			controller: 'SearchCtrl',
		})
		.state('otherwise', { // this is a name for our route
		  	url: '/*path', // encontrada actual url path of the route
		  	templateUrl: 'app/notfound/404.html', // the template that will load
...
.factory('Campaign', ['$resource', 'envService', function($resource, envService) {

  return $resource(envService.read('apiUrl') + '/campaign/:action/:slug/', {slug:'@slug'}, 
  {
	active: {
		method: 'GET',
...
.controller('CampaignCtrl', ['$scope', 'Campaign', function($scope, Campaign) {
	$scope.campaign = Campaign.active();

	var SafeArea = angular.element(window).height();
...
.factory('Company', ['$resource', 'envService', function ($resource, envService) {
   return $resource(envService.read('apiUrl') + '/companies/:slug/:action/', {slug:'@slug'}, {
       featured_home: {
       		method: 'GET', 
       		params: {action: 'featured_home'}, 
       		isArray: true 
       	},
        featured_offers: {
          method: 'GET', 
          params: {action: 'offers'}
        },
        featured_listing: {
          method: 'GET', 
          params: {action: 'featured_home'}, 
...
.controller('CompanyCtrl', function($scope, $http, $timeout, Company) {
  $scope.featured_companies = Company.featured_home();
})

.controller('CompanyListingCtrl', ['$scope', '$http', '$sce', '$state', '$stateParams', 'Product', 'Company', function($scope, $http, $sce, $state, $stateParams, Product, Company) {

  $scope.featured_companies_index = Company.featured_listing();

  $scope.seo.title = 'Veja aqui a listagem de farmácias parceiras.';
  $scope.seo.description = 'Nesta página você encontra as farmácias e drogarias parceiras MultiFarmas como por exemplo Ultrafarma e Droga Raia.';
  $scope.seo.breadcrumb = [
      {url: $state.href('home'), text: 'Início'},
      {url: $state.href('companies-listing'), text:'listagem de farmácias'},
      ];
  $scope.seo.indextag = "index, follow";
  
}])

.controller('CompanyDetailsCtrl', ['$scope', '$http', '$sce', '$state', '$stateParams', 'Product', 'Company', function($scope, $http, $sce, $state, $stateParams, Product, Company){
  
  // Initializing variables
  $scope.slug = $stateParams.slug;
  $scope.query = $stateParams.q;
  $scope.display_loading_screen = false;

  $scope.company = Company.get({slug: $scope.slug}, function(){

    $scope.seo.title = "Detalhes da " + $scope.company.name;
    $scope.seo.description = "Nesta página você encontra detalhes da rede " + $scope.company.name + " como endereços e principais ofertas para comprar seus remédios.";
    $scope.seo.breadcrumb = [
      {url: $state.href('home'), text: 'Início'},
      {url: $state.href('companies-listing'), text:'listagem de farmácias'},
      {url: $state.href('companies-details', {slug: $scope.slug}), text:"Detalhes - " + $scope.company.name},
      ];
    $scope.seo.indextag = "index, follow";
    $scope.seo.canonical = "http://www.multifarmas.com.br/lista-de-farmacias/"+$scope.slug;

});

}])

.controller('CompanyOffersCtrl', ['$scope', '$http', '$sce', '$state', '$stateParams', 'Product', 'Company', function($scope, $http, $sce, $state, $stateParams, Product, Company){
  ...
  $scope.featured_offers = Company.featured_offers({slug:$scope.slug, q:$scope.query});
...
.controller('MainCtrl', ['$scope', 'Campaign', 'Company', '$sce', function ($scope, Campaign, Company, $sce) {

    $scope.campaign = Campaign.active(function(campaign) {
        campaign.settings = campaign.settings.pop();
        campaign.settings.home_video_url = $sce.trustAsResourceUrl(campaign.settings.home_video_url);
	    //detectar o height da tela do usuario para adaptar o heigt do background da home	 
		var SafeArea = angular.element(window).height();
		if (angular.element(window).width() > 765){
			angular.element('.content-search').height(SafeArea-360);
			angular.element('.content-search').css('background-image','url( '+campaign.settings.home_background_img+' )');
		}
    });

    $scope.seo.title = 'Multifarmas | Compare Preços de Remédios e Medicamentos';
    $scope.seo.description = 'Compare preços de remédios buscando genéricos, similares, principios ativos em farmácias online, leia bula e encontre farmácias próximas a você';
    $scope.seo.breadcrumb = [];
    $scope.seo.indextag = "index, follow";
...
.factory('Newsletter', ['$resource', 'envService', function($resource, envService) {
	return $resource(envService.read('apiUrl') + '/users/:user/newsletter/:action/', {user:'@user'}, {
		save: {
			method: 'POST',
			params: {
				action: 'optin'
...
.controller('NewsletterCtrl', ['$scope','$http', 'Newsletter', 'md5','$timeout', function($scope, $http, Newsletter, md5, $timeout) {
	...
	//get user ip
	check_ip_service = 'http://ipv4.myexternalip.com/json';
	$http.get(check_ip_service).then(function(result) {
		$scope.newsletter.user_ip = result.data.ip;
	}, function(errors) {
	    console.log('Não foi possivel pegar o ip do usuário');
	    console.log(errors);
	});
	...
	$scope.SubmitForm = function() {
		...
		if ($scope.newsletter.user_email > 1) {
			user_hash = md5.createHash($scope.newsletter.user_email)			
		}
		...
		Newsletter.save(
			...
			function(response, headers) {
				$timeout( function(){
				 $scope.input_form_has_error=false;				
				  $scope.success_message=true;  
				}, 100);
			},	
			function(errors) {
				$timeout( function(){
					$scope.input_form_has_error=true;
					$scope.error_messages_user ="Preencha todos os campos";
				}, 100);
...
.factory('Product', ['$resource', 'envService', function ($resource, envService) {
   return $resource(envService.read('apiUrl') + '/products/:slug/:action/:letter/', {slug:'@slug', letter:'@letter'}, {
   		leaflet: {
   			method: 'GET',
   			params: {
   				action: 'leaflet'
...
.controller('NotFoundPageCtrl', ['$scope', '$http', '$sce', '$state', '$stateParams', 'Product', 'Company', function($scope, $http, $sce, $state, $stateParams, Product, Company) {
	
	$scope.seo.title = 'Multifarmas | Opa, esta rota não é conhecida.';
    $scope.seo.description = 'Esta URL não é conhecida talvez possa ir para home e tentar novamente.';
    $scope.seo.indextag = "noindex, nofollow";
}])  

.controller('ProductListByFirstLetterIndexCtrl', ['$scope', '$http', '$sce', '$state', '$stateParams', 'Product', 'Company', function($scope, $http, $sce, $state, $stateParams, Product, Company) {
	
	$scope.seo.title = 'Multifarmas | Listagem de Remédios por ordem alfabética.';
    $scope.seo.description = 'Encontre o seu remédio navegando pelos nomes dos produtos em orgem alfabética.';
    $scope.seo.breadcrumb = [
			{url: $state.href('home'), text: 'Início'},
			{url: $state.href('product-list-index'), text:'listagem de remédios'},
			];
	$scope.seo.indextag = "index, follow";
}])

.controller('ProductListByFirstLetterCtrl', ['$scope', '$http', '$sce', '$state', '$stateParams', 'Product', 'Company', function($scope, $http, $sce, $state, $stateParams, Product, Company) {
	...
	$scope.chosen_letter = $stateParams.letter;


    $scope.fetch_by_first_letter = function(letter) {
	    return Product.fetch_by_first_letter({
	    	letter: letter
	    }, function(response, headers) {
    ...
    if ($scope.chosen_letter != '') {
    	$scope.search = $scope.fetch_by_first_letter($scope.chosen_letter);
    } else {
    	$scope.search = [];
    }
    $scope.seo.title = 'Multifarmas | Listagem de Remédios com a letra ' + $scope.chosen_letter + '.';
    $scope.seo.description = 'Encontre o seu remédio que inicia com a letra '  + $scope.chosen_letter + '.';
    $scope.seo.breadcrumb = [
			{url: $state.href('home'), text: 'Início'},
			{url: $state.href('product-list-index'), text:'lista de remédios'},
			{url: $state.href('product-list-by-first-letter'), text: 'Remédios com ' + $scope.chosen_letter},
		];
	$scope.seo.indextag = "index, follow";
}])
.controller('ProductCtrl', ['$scope', '$http', '$sce', '$state', '$stateParams', 'Product', 'Company', function($scope, $http, $sce, $state, $stateParams, Product, Company){

    $scope.seo.indextag = "index, follow";
    $scope.product_reminder = {};

    $scope.submit_product_reminder_form = function() {

    	$scope.product_reminder = Product.reminder(
    		{slug:$scope.product.objectID},
    		$scope.product_reminder,
			function(response, headers) {
				console.log('submit_product_reminder_form success');
				$scope.SuccessReminder = true;
				$scope.close_reminder = false;			
			},	
			function(response, errors) {
				...
				angular.forEach(response.data, function(value, key) {
					$scope.errorReminder = value;
				});
				angular.element(".last_content_form .back_reminder").css({"border":"2px solid red"});
			}	    		
    	);
    }

	$scope.get_title = function(content) {
		if (content.length <= 33) {
			return 'Menor preço para ' + content + ' na MultiFarmas';
		} else {
			return 'Menor preço para ' + content;
		}
	}

	$scope.get_description = function(content) {
		if (content.length <= 33) {
			return 'Na MultiFarmas você encontra o menor preço para o remédio ' + content + ' nas melhores farmácias online.';
		} else {
			return 'O menor preço para o ' + content + ' está aqui no MultiFarmas. Consulte a bula e encontre as melhores ofertas.';
		}
	}	

	$scope.slug = $stateParams.slug;

	$scope.leaflet_trusted_html_fields = [
		'prescription', 
		'pharmaceutical_form',
		'composition',
		'patient_information',
	...
	$scope.product = Product.get({slug:$scope.slug},function(product){

    	$scope.seo.canonical = "http://www.multifarmas.com.br/"+product.objectID;
    	$scope.seo.title = $scope.get_title(product.name);
    	$scope.seo.description = $scope.get_description(product.name);
    	if (product.short_name != '') {
    		second_level_breadcrumb = product.name_starts_with;
    	} else {
    		second_level_breadcrumb = product.name.split(' ')[0].toLowerCase();
    	}
		$scope.seo.breadcrumb = [
			{url: $state.href('home'), text: 'Início'},
			{url: $state.href('product-list-by-first-letter', {letter: second_level_breadcrumb}), text: 'Remédios com ' + second_level_breadcrumb},
			{url: $state.href('product-detail', {slug: $scope.slug }), text: product.name.toLowerCase()}
		];

		if (product.has_leaflet) {
			product.leaflet = Product.leaflet({slug:$scope.slug}, function(leaflet) {
				angular.forEach(leaflet, function(value, key) {
					if ($scope.leaflet_trusted_html_fields.indexOf(key) == -1) {
						delete leaflet[key];
					}
				});
			});
		}
		angular.forEach(product.offers,function(offer){
			// substitui a propriedade seller (slug) com o objeto do vendedor da oferta
			offer.seller = Company.get({
				slug: offer.seller
			});
		});
	...
	//dict para limitar caracteres do produto
	$scope.char_limit = {
	   'product_name': 35,
	   'product_used_for': 230,
	   'product_manufacturer':15,
	   'product_preparation':10,
	}

	//cadastro de lembrete mobile
	$scope.NextReminder = function(){
		angular.element(".first_content_form").css('display','none');
		angular.element(".last_content_form").css('display','inline-block');
	},
	$scope.BackReminder = function(){
		angular.element(".first_content_form").css('display','inline-block');
		angular.element(".last_content_form").css('display','none');
	}
...
.factory('Search', ['$resource', 'envService', function($resource, envService) {
	return $resource(envService.read('apiUrl') + '/products/search/', {}, {});
}])

.controller('SearchCtrl', function($scope, $http, $stateParams, $state, Search){
	
	$scope.get_title = function(content) {
		if (content.length <= 13) {
			return 'Melhor preço para ' + content + ' | Como Usar, bula e pra que serve';
		} else {
			return 'Melhor preço para ' + content + ' | Multifarmas';
		}
	}

	$scope.get_description = function(content) {
		if (content.length <= 13) {
			return 'Opa, encontramos o menor preço para o ' + content + ' bem aqui no MultiFarmas! Confira as melhores ofertas em diversas farmácias e garanta sua economia.';
		} else {
			return 'Opa, encontramos o menor preço para o ' + content + ' bem aqui no MultiFarmas! Confira as melhores ofertas em diversas farmácias';
		}
	}	

	$scope.query = "";
	$scope.active_ingredient = false;
	$scope.master_filter = false;
	$scope.used_filters = {};
	$scope.available_filters = [
		{name: 'type', label: 'Tipo', display: true},	
		{name: 'departament', label: 'Departamento', display: true},
		{name: 'section', label: 'Sessão', display: true},
		{name: 'category', label: 'Categoria', display: true},
		{name: 'preparation', label: 'Apresentação', display: true},
		{name: 'manufacturer', label: 'Fabricante', display: true},
	];
	$scope.original_results = [];
	$scope.slidevalue = {};
	$scope.slider = {};
	$scope.price_changed = false;
	$scope.generic_similar = 0;
	$scope.parameters = {};
	$scope.char_limit = {
	   'product_name': 65,
	   'product_used_for': 230,
	   'product_manufacturer':15,
	   'product_preparation':15,
	   'product_filter':20,
	}
	$scope.display_loading_screen = false;

	$scope.set_master_filter = function(filter_name) {
		keep_loop = true;
		angular.forEach($scope.search.results_filters, function(filter, category) {
			if (keep_loop) {
				if (category == filter_name) {
					if ($scope.master_filter == false) {
						$scope.master_filter = {};
						$scope.master_filter[category] = filter;
					}
					keep_loop = false;
				}				
			} 
		});
		//console.log($scope.master_filter);
	};

	$scope.fetch_results_filters = function(){
        var list = {};
        var is_selected;
        angular.forEach($scope.search.results, function (result, index) {
            angular.forEach(result, function(value, key) {
				angular.forEach($scope.available_filters, function(filter) {
					if (key == filter.name) {
						//if ($scope.generic_similar)
	            		if (list[filter.name] == undefined) {
	            			list[filter.name] = {};
	            		}
	            		
	            		is_selected = false;
	            		if ($scope.used_filters[filter.name] != undefined) {
	            			is_selected = $scope.used_filters[filter.name].indexOf(value) > -1;	
	            		}
	            		
	            		//if ()
			            if (list[filter.name][value] === undefined) {
			            	...
			            	list[filter.name][value] = {
			            		value: value,
			            		total: list[filter.name][value].total + 1,
			            		is_selected: is_selected
			            	};
			            }  		  
					}
        ...
        // caso o filtro mestre esteja definido, altera os filtros para os valores originais
        // para mantermos os resultados e que o usuário possa escolher multiplos valores para o filtro mestre
		if ($scope.master_filter != false) {
			keep_loop = true;
			angular.forEach(list, function(filter, category) {
				if (keep_loop) {
					if (Object.keys($scope.master_filter).indexOf(category) > -1) {
						list[category] = $scope.master_filter[category];
						keep_loop = false;
					}						
				}
		...
		$scope.search.results_filters = list; 

	};		

    $scope.fetch_results = function(){

    	$scope.display_loading_screen = true;

    	$scope.parameters = $scope.used_filters;
    	if ($scope.price_changed) {
     		$scope.parameters.max_price = $scope.slider['maxValue'];
    		$scope.parameters.min_price = $scope.slider['minValue'];
    	}
    	$scope.parameters.q = $scope.query;

		Search.get($scope.parameters, function(response) {
			$scope.search = response;
			$scope.fetch_results_filters();


			// Slide Filtro	
			$scope.slider = {
			    minValue: $scope.search.min_price,
			    maxValue: $scope.search.max_price,
			    options: {
			    	ceil: $scope.search.max_price,
			    	floor: $scope.search.min_price,
			    	precision: 2,
			    	step: 0.01,
			    	onEnd: function(id) {
			    		$scope.price_changed = true;
			    		$scope.fetch_results();
			    	},
			...
			console.log(['Erro ao fazer a busca', errors]);
		}).$promise.then(function(response) {
			if (!$state.is('search-with-product-shortname')) {
				$state.go('search', {q: $scope.search.query}, {location: true, inherit:true, notify:false});						
			}
	    	$scope.display_loading_screen = false;
		});
	...
	$scope.add_filter = function (id, value){
		keys = Object.keys($scope.used_filters);
		if (keys.length == 1) {
			$scope.set_master_filter(id);
		}				
		if ($scope.used_filters[id] == undefined) {
			$scope.used_filters[id] = [];
		}
		$scope.used_filters[id].push(value);
	};

	$scope.remove_filter = function(id, value){
		
		for (x=0; x < $scope.used_filters[id].length; x++) {
			if (value == $scope.used_filters[id][x]) {
				$scope.used_filters[id].pop(x);
			}
		}
		if ($scope.used_filters[id].length < 1) {
			delete $scope.used_filters[id];
		}

		keys = Object.keys($scope.used_filters);

		if (keys.indexOf('q') < 0 && keys.length < 1) {
			$scope.master_filter = '';
		}

	};
	
	$scope.apply_filter = function(id, value, $event){
		if ($event) {
			var checkbox = $event.target;	
		}
		
		if(checkbox && checkbox.checked == false){
			$scope.remove_filter(id, value);
		}else{
			$scope.add_filter(id, value);
		}
		$scope.fetch_results();
	}

	$scope.toogle_filter_generic_similar = function($event){
		if ($scope.generic_similar) {
			$scope.generic_similar = 0;
			$scope.used_filters = {}; 
	   		$scope.query = $scope.original_query;
		} else {
			$scope.generic_similar = 1;
			$scope.original_query = $scope.query;
			$scope.used_filters = {'type':['Genérico']}; 
	   		$scope.query = $scope.search.results[0].active_ingredient;
		}
		$scope.fetch_results();			

	}

	angular.element('.lightbox').css('display', 'none');
	$scope.OpenFilter = function(){
		angular.element('.filter').css('display', 'block');
		if(angular.element('.filter').is(':hidden')){
		}else{
			angular.element('.lightbox').css('display', 'block');
		}
	}
	$scope.CloseFilter = function(){
		angular.element('.lightbox').css('display', 'none');
		angular.element('.filter').css('display', 'none');
	}
	...
	$scope.$watch('query', function(newValue, oldValue){
		...
		$scope.seo.title = $scope.get_title(newValue);
		$scope.seo.description = $scope.get_description(newValue);
		$scope.seo.breadcrumb = [
			{url: $state.href('home'), text: 'Início'},
			{url: $state.href('search', {q: newValue}), text: 'Busca por ' + newValue},
		]
	});
...
.controller('TermsOfUseCtrl', ['$scope', '$http', '$sce', '$state', '$stateParams', 'Product', 'Company', function($scope, $http, $sce, $state, $stateParams, Product, Company) {
	
	$scope.seo.title = 'Multifarmas | Conheça nossos termos de uso.';
    $scope.seo.description = 'Nesta página você verá quais são nossos termos de uso e critérios adotados para eles.';
    $scope.seo.breadcrumb = [
			{url: $state.href('home'), text: 'Início'},
			{url: $state.href('terms-of-usage'), text:'Termos de uso'}
			];
}])

.controller('PrivacyCtrl', ['$scope', '$http', '$sce', '$state', '$stateParams', 'Product', 'Company', function($scope, $http, $sce, $state, $stateParams, Product, Company) {
	
	$scope.seo.title = 'Multifarmas | Conheça nossas politicas de privacidade.';
    $scope.seo.description = 'Veja como trabalhamos para garantir sua privacidade e até onde nos comprometemos com ela.';
    $scope.seo.breadcrumb = [
			{url: $state.href('home'), text: 'Início'},
			{url: $state.href('privacy-policy'), text:'Politica de privacidade'}
			];
}])

.controller('PubliCtrl', ['$scope', '$http', '$sce', '$state', '$stateParams', 'Product', 'Company', function($scope, $http, $sce, $state, $stateParams, Product, Company) {
	
	$scope.seo.title = 'Multifarmas | Conheça nossas politicas de publicidade.';
    $scope.seo.description = 'Veja como trabalhamos para nossas divulgações e publicidades.';
    $scope.seo.breadcrumb = [
			{url: $state.href('home'), text: 'Início'},
			{url: $state.href('publi-policy'), text:'Politica de publicidade'}
			];
}])

.controller('WhoWeAreCtrl', ['$scope', '$http', '$sce', '$state', '$stateParams', 'Product', 'Company', function($scope, $http, $sce, $state, $stateParams, Product, Company) {
	
	$scope.seo.title = 'Multifarmas | Quem somos.';
    $scope.seo.description = 'Conheça mais sobre nós.';
    $scope.seo.breadcrumb = [
			{url: $state.href('home'), text: 'Início'},
			{url: $state.href('who-we-are'), text:'Quem somos'}
			];
}])

.controller('OurPartnersCtrl', ['$scope', '$http', '$sce', '$state', '$stateParams', 'Product', 'Company', function($scope, $http, $sce, $state, $stateParams, Product, Company) {
	
	$scope.seo.title = 'Multifarmas | Nossos Parceiros.';
    $scope.seo.description = 'Veja quais são nossos parceiros.';
    $scope.seo.breadcrumb = [
			{url: $state.href('home'), text: 'Início'},
			{url: $state.href('our-partners'), text:'Nossos parceiros'}
			];
}])

.controller('ContactUsCtrl', ['$scope', '$http', '$sce', '$state', '$stateParams', 'Product', 'Company', function($scope, $http, $sce, $state, $stateParams, Product, Company) {
	
	$scope.seo.title = 'Multifarmas | Entre em contato.';
    $scope.seo.description = 'Fale Conosco.';
    $scope.seo.breadcrumb = [
			{url: $state.href('home'), text: 'Início'},
			{url: $state.href('contact-us'), text:'Entre em contato'}
			];
}])

.controller('MidiaWeHaveBeenCtrl', ['$scope', '$http', '$sce', '$state', '$stateParams', 'Product', 'Company', function($scope, $http, $sce, $state, $stateParams, Product, Company) {
	
	$scope.seo.title = 'Multifarmas | Onde aparecemos na mídia.';
    $scope.seo.description = 'Veja quais veículos já divulgaram alguma matéria sobre nós.';
    $scope.seo.breadcrumb = [
			{url: $state.href('home'), text: 'Início'},
			{url: $state.href('contact-us'), text:'Na mídia'}
			];
}])
